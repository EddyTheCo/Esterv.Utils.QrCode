...
QPixmap QRImageProvider::requestPixmap(const QString &id, QSize *size,
  ...
  const auto max = (requestedSize.width() > requestedSize.height())
                       ? requestedSize.width()
                       : requestedSize.height();

  QPixmap pixmap(max > 0 ? max : width, max > 0 ? max : width);
  pixmap.fill(Qt::transparent);
  *size = pixmap.size();
  const QrCode qr = QrCode::encodeText(data.toStdString().c_str(),
                                       static_cast<QrCode::Ecc>(errC));
